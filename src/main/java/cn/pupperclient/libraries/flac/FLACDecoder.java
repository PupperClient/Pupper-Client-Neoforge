...
import cn.pupperclient.libraries.flac.frame.*;
import cn.pupperclient.libraries.flac.io.BitInputStream;
import cn.pupperclient.libraries.flac.io.RandomFileInputStream;
import cn.pupperclient.libraries.flac.metadata.*;
...
public class FLACDecoder {
	 ...
	 * @return the ByteData that was filled (may be a new instance from
	 *         <code>space</code>)
	 */
	 ...
	 * 
	 * while (true) { switch (state) { case DECODER_SEARCH_FOR_METADATA :
	 * readStreamSync(); break; case DECODER_READ_METADATA : readNextMetadata(); //
	 * above function sets the status for us return true; case
	 * DECODER_SEARCH_FOR_FRAME_SYNC : frameSync(); // above function sets the
	 * status for us break; case DECODER_READ_FRAME : readFrame(); return true; //
	 * above function sets the status for us //break; case DECODER_END_OF_STREAM :
	 * case DECODER_ABORTED : return true; default : return false; } } }
	 */
	 ...
	 * while (true) { switch (state) { case DECODER_SEARCH_FOR_METADATA :
	 * readStreamSync(); break; case DECODER_READ_METADATA : readNextMetadata(); //
	 * above function sets the status for us break; case
	 * DECODER_SEARCH_FOR_FRAME_SYNC : case DECODER_READ_FRAME : case
	 * DECODER_END_OF_STREAM : case DECODER_ABORTED : default : return; } } }
	 */
	...
	public SeekPoint seek(long target_sample) throws IOException {
		...
		long pos = -1;
		/*
		 * If there's no seek table, we need to use the metadata (if we have it) and the
		 * filelength to estimate the position of the frame with the correct sample.
		 */
		if (total_samples > 0) {
			/*
			 * For max accuracy we should be using (stream_length-first_frame_offset-1) in
			 * the divisor, but the difference is trivial and
			 * (stream_length-first_frame_offset) has no chance of underflow.
			 */
			pos = first_frame_offset + ((target_sample * (stream_length - first_frame_offset)) / total_samples)
					- approx_bytes_per_frame;
		}
		/*
		 * If there's no seek table and total_samples is unknown, we don't even bother
		 * trying to figure out a target, we just use our current position.
		 */
		int i = 0;
		...
		while (true) {
			...
			/*
			 * Now we need to get a frame. It is possible for our seek to land in the middle
			 * of audio data that looks exactly like a frame header from a future version of
			 * an encoder. When that happens, frame_sync() will return false. But there is a
			 * remote possibility that it is properly synced at such a "future-codec frame",
			 * so to make sure, we wait to see several "unparseable" errors in a row before
			 * bailing out.
			 */
			...
			if (target_sample >= this_frame_sample && target_sample < this_frame_sample + this_block_size) {
				...
				if (this_frame_sample - target_sample <= this_block_size * 10) {
					/*
					 * Target is no more than 10 frames back, seek backwards a frame at a time.
					 */
					if (__SEEK_DEBUG)
					...
					/*
					 * Target may be more than 10 frames back, calculated new seek position.
					 */
					last_pos = pos;
				...
				if (target_sample - this_frame_sample <= min_blocksize * 10) {
					...
					/*
					 * Target is no more than 10 frames ahead, seek forwards a frame at a time.
					 */
					...
					// just keep reading
					/*
					 * If we haven't hit the target frame yet and our position hasn't changed, it
					 * means we're at the end of the stream and the seek target does not exist.
					 */
				} else {
					/*
					 * Target may be more than 10 frames ahead, calculated new seek position.
					 */
					...
					long max_bytes_to_frame = ((target_sample - this_frame_sample + min_blocksize - 1) / min_blocksize)
							* max_framesize;
					long delta = this_frame_sample - last_frame_sample;
					// System.err.printf("del %d, thi %d, las %d lasj %d%n", delta,
					// this_frame_sample,
					...
					pos = rf.getPosition() + this_jump;
					if (__SEEK_DEBUG)
						System.err.printf("Need a jump forward samples  %d for %d to %d%n",
								target_sample - this_frame_sample, this_jump, target_sample);
					last_jump = this_jump;
					needs_seek = true;
				}
			}
	...
	/*
	 * private boolean processUntilEndOfStream() throws IOException { //boolean
	 * got_a_frame;
	 * 
	 * while (true) { switch (state) { case DECODER_SEARCH_FOR_METADATA :
	 * readStreamSync(); break; case DECODER_READ_METADATA : readNextMetadata(); //
	 * above function sets the status for us break; case
	 * DECODER_SEARCH_FOR_FRAME_SYNC : frameSync(); // above function sets the
	 * status for us //System.exit(0); break; case DECODER_READ_FRAME : readFrame();
	 * break; case DECODER_END_OF_STREAM : case DECODER_ABORTED : return true;
		...
		try {
			while (true) {
				...
				try {
					...
					// e.printStackTrace();
					// System.err.print("Error "+e+" at sample "+samplesDecoded+" of
					// "+streamInfo.getTotalSamples());
					badFrames++;
				}
				// break;
				// case DECODER_END_OF_STREAM :
	...
	/*
	 * public int getInputBytesUnconsumed() { return is.getInputBytesUnconsumed(); }
	 */
	...
	private void findFrameSync() throws IOException {
		...
		// If we know the total number of samples in the stream, stop if we've read that
		// many.
		// This will stop us, for example, from wasting time trying to sync on an ID3V1
		// tag.
		...
		try {
			while (true) {
				...
				if (x == 0xff) { // MAGIC NUMBER for the first 8 frame sync bits
					...
					/*
					 * we have to check if we just read two 0xff's in a row; the second may actually
					 * be the beginning of the sync code
					 */
	...
	public void readFrame() throws IOException, FrameDecodeException {
		...
		if (frameCRC == frame.getCRC()) {
			...
			frameListeners.processError("CRC Error: " + Integer.toHexString((frameCRC & 0xffff)) + " vs "
					+ Integer.toHexString((frame.getCRC() & 0xffff)));
			for (channel = 0; channel < frame.header.channels; channel++) {
		...
		// System.out.println(samplesDecoded+" "+frame.header.sampleNumber + "
	...
	private void readSubframe(int channel, int bps) throws IOException, FrameDecodeException {
		...
		if ((x & 0x80) != 0) {
			...
			frame.subframes[channel] = new ChannelConstant(bitStream, frame.header, channelData[channel], bps,
					wastedBits);
		} else if (x == 2) {
			frame.subframes[channel] = new ChannelVerbatim(bitStream, frame.header, channelData[channel], bps,
					wastedBits);
		} else if (x < 16) {
			// state = DECODER_UNPARSEABLE_STREAM;
			throw new FrameDecodeException("ReadSubframe Bad Subframe Type: " + Integer.toHexString(x & 0xff));
		} else if (x <= 24) {
			// FLACSubframe_Fixed subframe = read_subframe_fixed_(channel, bps, (x >> 1) &
			// 7);
			frame.subframes[channel] = new ChannelFixed(bitStream, frame.header, channelData[channel], bps, wastedBits,
					(x >> 1) & 7);
		} else if (x < 64) {
			// state = DECODER_UNPARSEABLE_STREAM;
			throw new FrameDecodeException("ReadSubframe Bad Subframe Type: " + Integer.toHexString(x & 0xff));
		} else {
			frame.subframes[channel] = new ChannelLPC(bitStream, frame.header, channelData[channel], bps, wastedBits,
					((x >> 1) & 31) + 1);
		}
		if (haveWastedBits) {
			int i;
