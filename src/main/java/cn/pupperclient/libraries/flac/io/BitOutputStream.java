package cn.pupperclient.libraries.flac.io;
...
import cn.pupperclient.libraries.flac.util.CRC16;
import cn.pupperclient.libraries.flac.util.CRC8;
...
public class BitOutputStream {
	...
	private static final long[] MASK32 = new long[] { 0, 0x0000000000000001, 0x0000000000000003, 0x0000000000000007,
			0x000000000000000F, 0x000000000000001F, 0x000000000000003F, 0x000000000000007F, 0x00000000000000FF,
			0x00000000000001FF, 0x00000000000003FF, 0x00000000000007FF, 0x0000000000000FFF, 0x0000000000001FFF,
			0x0000000000003FFF, 0x0000000000007FFF, 0x000000000000FFFF, 0x000000000001FFFF, 0x000000000003FFFF,
			0x000000000007FFFF, 0x00000000000FFFFF, 0x00000000001FFFFF, 0x00000000003FFFFF, 0x00000000007FFFFF,
			0x0000000000FFFFFF, 0x0000000001FFFFFF, 0x0000000003FFFFFF, 0x0000000007FFFFFF, 0x000000000FFFFFFF,
	...
	/*
	 * WATCHOUT: The current implentation is not friendly to shrinking, i.e. it does
	 * not shift left what is consumed, it just chops off the end, whether there is
	 * unconsumed data there or not. This is OK because currently we never shrink
	 * the buffer, but if this ever changes, we'll have to do some fixups here.
	 */
	private boolean resize(int newCapacity) {
		if (outCapacity >= newCapacity)
			return true;
	...
	/*
	 * private int readFromStream() throws IOException { // first shift the
	 * unconsumed buffer data toward the front as much as possible if
	 * (totalConsumedBits >= BITS_PER_BLURB) { int l = 0; int r = consumedBlurbs;
	 * int r_end = inBlurbs + ((inBits != 0) ? 1 : 0); for (; r < r_end; l++, r++)
	 * buffer[l] = buffer[r]; for (; l < r_end; l++) buffer[l] = 0; inBlurbs -=
	 * consumedBlurbs; totalBits -= consumedBlurbs << 3; consumedBlurbs = 0;
	 * totalConsumedBits = consumedBits; }
	 * 
	 * // grow if we need to if (capacity <= 1) resize(16);
	 * 
	 * // set the target for reading, taking into account blurb alignment // blurb
	 * == byte, so no gyrations necessary: int bytes = capacity - inBlurbs;
	 * 
	 * // finally, read in some data bytes = is.read(buffer, inBlurbs, bytes); if
	 * (bytes <= 0) throw new EOFException();
	 * 
	 * // now we have to handle partial blurb cases: // blurb == byte, so no
	 * gyrations necessary: inBlurbs += bytes; totalBits += bytes << 3; return
	 * bytes; }
	 */
	...
	public boolean concatenateAligned(BitOutputStream src) {
		...
		if (bitsToAdd == 0)
			return true;
		if (outBits != src.consumedBits)
			return false;
		if (!ensureSize(bitsToAdd))
			return false;
		if (outBits == 0) {
	...
	public void writeRawUInt(int val, int bits) throws IOException {
		...
		// zero-out unused bits; WATCHOUT: other code relies on this, so this needs to
		// stay
		if (bits < 32)
		...
		while (bits > 0) {
			...
			if (n == BITS_PER_BLURB) { // i.e. outBits == 0
				...
				// we know n > 0 so k < 32 so no need to protect against the gcc bug mentioned
				// above
				val &= (~(0xffffffff << k));
				bits -= n;
				outBlurbs++;
	...
	public void writeRawULong(long val, int bits) throws IOException {
		...
		while (bits > 0) {
			if (outBits == 0) {
				...
				// we know n > 0 so k < 64 so no need to protect against the gcc bug mentioned
				// above
				val &= (~(0xffffffffffffffffL << k));
				bits -= n;
				outBits += n;
	...
	public int riceBits(int val, int parameter) {
		...
		if (val < 0) {
			// equivalent to (unsigned)(((--val) < < 1) - 1); but without the overflow
			// problem at MININT
			uval = (int) (((-(++val)) << 1) + 1);
		} else {
			uval = (int) (val << 1);
	...
	/*
	 * DRR FIX # ifdef SYMMETRIC_RICE boolean write_symmetric_rice_signed(BitBuffer8
	 * * bb, int val, unsigned parameter) { unsigned total_bits, interesting_bits,
	 * msbs; uint32 pattern;
	 * 
	 * ASSERT(0 != bb); ASSERT(0 != buffer); ASSERT(parameter <= 31); // init
	 * pattern with the unary end bit and the sign bit if (val < 0) { pattern = 3;
	 * val = -val; } else pattern = 2;
	 * 
	 ...
	 * return false; } else { // write the unary MSBs if (!write_zeroes(bb, msbs))
	 * return false; // write the unary end bit, the sign bit, and binary LSBs if
	 * (!write_raw_uint32(bb, pattern, interesting_bits)) return false; } return
	 * true; }
	 * 
	 * boolean write_symmetric_rice_signed_escape(BitBuffer8 * bb, int val, unsigned
	 * parameter) { unsigned total_bits, val_bits; uint32 pattern;
	 * 
	 * ASSERT(0 != bb); ASSERT(0 != buffer); ASSERT(parameter <= 31);
	 * 
	 * val_bits = bitmath_silog2(val); total_bits = 2 + parameter + 5 + val_bits;
	 * 
	 * if (total_bits <= 32) { pattern = 3; pattern < <= (parameter + 5); pattern |=
	 * val_bits; pattern < <= val_bits; pattern |= (val & ((1 < < val_bits) - 1));
	 * if (!write_raw_uint32(bb, pattern, total_bits)) return false; } else { //
	 * write the '-0' escape code first if (!write_raw_uint32(bb, 3 u < < parameter,
	 * 2 + parameter)) return false; // write the length if (!write_raw_uint32(bb,
	 * val_bits, 5)) return false; // write the value if (!write_raw_int32(bb, val,
	 * val_bits)) return false; } return true; } # endif // ifdef SYMMETRIC_RICE
	 */
	 ...
	 * (buffer[consumed_blurbs] & BLURB_BIT_TO_MASK(consumed_bits)) ? 1 : 0; return
	 * true; } else { if (!read_from_client_(bb, read_callback, client_data)) return
	 * false; } } }
	 */

	/*
	 * # ifdef SYMMETRIC_RICE boolean read_symmetric_rice_signed( BitBuffer8 * bb,
	 * int * val, unsigned parameter, boolean(* read_callback) (byte buffer[],
	 * unsigned * bytes, void * client_data), void * client_data) { uint32 sign = 0,
	 * lsbs = 0, msbs = 0;
	 * 
	 * ASSERT(0 != bb); ASSERT(0 != buffer); ASSERT(parameter <= 31); // read the
	 * unary MSBs and end bit if (!read_unary_unsigned(bb, & msbs, read_callback,
	 * client_data)) return false; // read the sign bit if (!read_bit_to_uint32(bb,
	 * & sign, read_callback, client_data)) return false; // read the binary LSBs if
	 * (!read_raw_uint32(bb, & lsbs, parameter, read_callback, client_data)) return
	 * false; // compose the value val = (msbs < < parameter) | lsbs; if (sign) val
	 * = - (* val);
	 * 
	 ...
	 * client_data), void * client_data) { uint32 lsbs = 0, msbs = 0; unsigned uval;
	 * 
	 * ASSERT(0 != bb); ASSERT(0 != buffer); ASSERT(parameter <= 31); // read the
	 * unary MSBs and end bit if (!read_unary_unsigned(bb, & msbs, read_callback,
	 * client_data)) return false; // read the binary LSBs if (!read_raw_uint32(bb,
	 * & lsbs, parameter, read_callback, client_data)) return false; // compose the
	 * value uval = (msbs < < parameter) | lsbs; if (uval & 1) val = - ((int) (uval
	 * >> 1)) - 1; else val = (int) (uval >> 1);
	 * 
	 * return true; }
	 */
