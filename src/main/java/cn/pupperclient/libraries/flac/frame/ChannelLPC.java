package cn.pupperclient.libraries.flac.frame;
...
import cn.pupperclient.libraries.flac.ChannelData;
import cn.pupperclient.libraries.flac.FrameDecodeException;
import cn.pupperclient.libraries.flac.LPCPredictor;
import cn.pupperclient.libraries.flac.io.BitInputStream;
import cn.pupperclient.libraries.flac.util.BitMath;
...
public class ChannelLPC extends Channel {
	 ...
	 */
	public ChannelLPC(BitInputStream is, Header header, ChannelData channelData, int bps, int wastedBits, int order)
		...
		// read residual
		entropyCodingMethod.readResidual(is, order, entropyCodingMethod.order, header, channelData.getResidual());

		// System.out.println();
		// for (int i = 0; i < header.blockSize; i++)
		// {System.out.print(channelData.residual[i]+" ");
		...
		if (bps + qlpCoeffPrecision + BitMath.ilog2(order) <= 32) {
			if (bps <= 16 && qlpCoeffPrecision <= 16)
				LPCPredictor.restoreSignal(channelData.getResidual(), header.blockSize - order, qlpCoeff, order,
						quantizationLevel, channelData.getOutput(), order);
			else
				LPCPredictor.restoreSignal(channelData.getResidual(), header.blockSize - order, qlpCoeff, order,
						quantizationLevel, channelData.getOutput(), order);
		} else {
			LPCPredictor.restoreSignalWide(channelData.getResidual(), header.blockSize - order, qlpCoeff, order,
					quantizationLevel, channelData.getOutput(), order);
		}
	}
