...
import cn.pupperclient.libraries.flac.Constants;
...
public class Header {
	...
	/**
	 * CRC-8 (polynomial = x^8 + x^2 + x^1 + x^0, initialized with 0). of the raw
	 * frame header bytes, meaning everything before the CRC byte including the sync
	 * code.
	 */
	protected byte crc;
	 ...
	 */
	public Header(BitInputStream is, byte[] headerWarmup, StreamInfo streamInfo)
		...
		boolean isKnownVariableBlockSizeStream = (streamInfo != null
				&& streamInfo.getMinBlockSize() != streamInfo.getMaxBlockSize());
		boolean isKnownFixedBlockSizeStream = (streamInfo != null
				&& streamInfo.getMinBlockSize() == streamInfo.getMaxBlockSize());
	...
	public String toString() {
		return "FrameHeader:" + " BlockSize=" + blockSize + " SampleRate=" + sampleRate + " Channels=" + channels
				+ " ChannelAssignment=" + channelAssignment + " BPS=" + bitsPerSample + " SampleNumber=" + sampleNumber;
	}
}
