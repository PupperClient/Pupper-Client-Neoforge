...
public class Constants {
	...
	/*
	 * boolean format_sample_rate_is_valid(int sample_rate) { if (sample_rate == 0
	 * || sample_rate > MAX_SAMPLE_RATE || (sample_rate >= (1 << 16) &&
	 * !(sample_rate % 1000 == 0 || sample_rate % 10 == 0))) { return false; } else
	 * return true; }
	 */
	 ...
	 * format_get_max_rice_partition_order_from_blocksize(blocksize), blocksize,
	 * predictor_order); }
	 * 
	 * int format_get_max_rice_partition_order_from_blocksize(int blocksize) { int
	 * max_rice_partition_order = 0; while ((blocksize & 1) == 0) {
	 * max_rice_partition_order++; blocksize >>= 1; } return
	 * Math.min(MAX_RICE_PARTITION_ORDER, max_rice_partition_order); }
	 * 
	 * int
	 * format_get_max_rice_partition_order_from_blocksize_limited_max_and_predictor_order(
	 * int limit, int blocksize, int predictor_order) { int max_rice_partition_order
	 * = limit;
	 * 
	 * while (max_rice_partition_order > 0 && (blocksize >>
	 * max_rice_partition_order) <= predictor_order) max_rice_partition_order--;
	 * 
