...
package cn.pupperclient.libraries.material3.dynamiccolor;

import cn.pupperclient.libraries.material3.contrast.Contrast;
import cn.pupperclient.libraries.material3.hct.Hct;
import cn.pupperclient.libraries.material3.palettes.TonalPalette;
import cn.pupperclient.libraries.material3.utils.MathUtils;
...
public final class DynamicColor {
	 ...
	 *                         chroma, so this replaces the need to specify
	 *                         hue/chroma. By providing a tonal palette, when
	 *                         contrast adjustments are made, intended chroma can be
	 *                         preserved.
	 * @param tone             Function that provides a tone, given a DynamicScheme.
	 * @param isBackground     Whether this dynamic color is a background, with some
	 *                         other color as the foreground.
	 * @param background       The background of the dynamic color (as a function of
	 *                         a `DynamicScheme`), if it exists.
	 * @param secondBackground A second background of the dynamic color (as a
	 *                         function of a `DynamicScheme`), if it exists.
	 * @param contrastCurve    A `ContrastCurve` object specifying how its contrast
	 *                         against its background should behave in various
	 *                         contrast levels options.
	 * @param toneDeltaPair    A `ToneDeltaPair` object specifying a tone delta
	 *                         constraint between two colors. One of them must be
	 *                         the color being constructed.
	 */
	public DynamicColor(String name, Function<DynamicScheme, TonalPalette> palette,
			Function<DynamicScheme, Double> tone, boolean isBackground,
			Function<DynamicScheme, DynamicColor> background, Function<DynamicScheme, DynamicColor> secondBackground,
	 ...
	 *                         constraint between two colors. One of them must be
	 *                         the color being constructed.
	 * @param opacity          A function returning the opacity of a color, as a
	 ...
	 *                TonalPalette is defined by a hue and chroma, so this replaces
	 *                the need to specify hue/chroma. By providing a tonal palette,
	 *                when contrast adjustments are made, intended chroma can be
	 *                preserved.
	 * @param tone    Function that provides a tone, given a DynamicScheme.
	 */

	public static DynamicColor fromPalette(String name, Function<DynamicScheme, TonalPalette> palette,
			Function<DynamicScheme, Double> tone) {
		return new DynamicColor(name, palette, tone, /* isBackground= */ false, /* background= */ null,
	 ...
	 * @param palette      Function that provides a TonalPalette given
	 *                     DynamicScheme. A TonalPalette is defined by a hue and
	 *                     chroma, so this replaces the need to specify hue/chroma.
	 *                     By providing a tonal palette, when contrast adjustments
	 *                     are made, intended chroma can be preserved.
	 * @param tone         Function that provides a tone, given a DynamicScheme.
	 * @param isBackground Whether this dynamic color is a background, with some
	 *                     other color as the foreground.
	 */
	...
	public static DynamicColor fromPalette(String name, Function<DynamicScheme, TonalPalette> palette,
			Function<DynamicScheme, Double> tone, boolean isBackground) {
		return new DynamicColor(name, palette, tone, isBackground, /* background= */ null, /* secondBackground= */ null,
	 ...
	 *
	 * <p>
	 * Result has no background; thus no support for increasing/decreasing contrast
	 * for a11y.
	 *
	 * @param name The name of the dynamic color.
	 ...
	 *               whether or not it is dark mode or light mode, and what the
	...
	public Hct getHct(DynamicScheme scheme) {
		...
		// color
		// and changing its tone for contrast. Rather, we find the tone for contrast,
		// then
		// use the specified chroma from the palette to construct a new color.
		//
		// For example, this enables colors with standard tone of T90, which has limited
		// chroma, to
	...
	/**
	 * Returns the tone in HCT, ranging from 0 to 100, of the resolved color given
	 * scheme.
	 */
	public double getTone(DynamicScheme scheme) {
		...
		if (toneDeltaPair != null) {
			...
			boolean aIsNearer = (polarity == TonePolarity.NEARER || (polarity == TonePolarity.LIGHTER && !scheme.isDark)
			...
			double nTone = Contrast.ratioOfTones(bgTone, nInitialTone) >= nContrast ? nInitialTone
					: DynamicColor.foregroundTone(bgTone, nContrast);
			// Initial and adjusted tones for `farther`
			double fInitialTone = farther.tone.apply(scheme);

			double fTone = Contrast.ratioOfTones(bgTone, fInitialTone) >= fContrast ? fInitialTone
			...
			if (secondBackground != null) {
				...
				boolean prefersLight = DynamicColor.tonePrefersLightForeground(bgTone1)
	...
	/**
	 * Given a background tone, find a foreground tone, while ensuring they reach a
	 * contrast ratio that is as close to ratio as possible.
	 */
	public static double foregroundTone(double bgTone, double ratio) {
		...
		if (preferLighter) {
			// "Neglible difference" handles an edge case where the initial contrast ratio
			// is high
			// (ex. 13.0), and the ratio passed to the function is that high ratio, and both
			// the lighter
			// and darker ratio fails to pass that ratio.
			//
			// This was observed with Tonal Spot's On Primary Container turning black
			// momentarily between
			// high and max contrast in light mode. PC's standard tone was T90, OPC's was
			// T10, it was
			// light mode, and the contrast level was 0.6568521221032331.
			boolean negligibleDifference = Math.abs(lighterRatio - darkerRatio) < 0.1 && lighterRatio < ratio
					&& darkerRatio < ratio;
	...
	/**
	 * People prefer white foregrounds on ~T60-70. Observed over time, and also by
	 * Andrew Somers during research for APCA.
	 *
	 * <p>
	 * T60 used as to create the smallest discontinuity possible when skipping down
	 * to T49 in order to ensure light foregrounds.
	 *
	 * <p>
	 * Since `tertiaryContainer` in dark monochrome scheme requires a tone of 60, it
	 * should not be adjusted. Therefore, 60 is excluded here.
	 */
