...
package cn.pupperclient.libraries.material3.temperature;
...
public final class TemperatureCache {
	 ...
	 *              have the same tone, and chroma as the input color, modulo any
	 *              restrictions due to the other hues having lower limits on
	 *              chroma.
	 */
	 ...
	 *
	 * <p>
	 * In art, this is usually described as being across the color wheel. History of
	 * this shows intent as a color that is just as cool-warm as the input color is
	 * warm-cool.
	 */
	 ...
	 * See: - Li-Chen Ou's Chapter 19 in Handbook of Color Psychology (2015). -
	 * Josef Albers' Interaction of Color chapters 19 and 21.
	 *
	 * <p>
	 * Implementation of Ou, Woodcock and Wright's algorithm, which uses Lab/LCH
	 * color space. Return value has these properties:<br>
	...
	public static double rawTemperature(Hct color) {
		...
		return -0.5
				+ 0.02 * Math.pow(chroma, 1.07) * Math.cos(Math.toRadians(MathUtils.sanitizeDegreesDouble(hue - 50.)));
	}
	 ...
	 */
	// Prevent lint for Comparator not being available on Android before API level
	// 24, 7.0, 2016.
	// "AndroidJdkLibsChecker" for one linter, "NewApi" for another.
	// A java_library Bazel rule with an Android constraint cannot skip these
	...
	private List<Hct> getHctsByTemp() {
		...
		hcts.add(input);
		Comparator<Hct> temperaturesComparator = Comparator.comparing((Hct arg) -> getTempsByHct().get(arg),
				Double::compareTo);
