...
package cn.pupperclient.libraries.material3.contrast;
...
public final class Contrast {
	...
	// Contrast ratio equation = lighter + 5 / darker + 5, if lighter == darker,
	// ratio == 1.
	public static final double RATIO_MIN = 1.0;

	// The maximum contrast ratio of two colors.
	// Contrast ratio equation = lighter + 5 / darker + 5. Lighter and darker scale
	// from 0 to 100.
	...
	// reaches that ratio
	// with the color can be calculated. However, that luminance may not contrast as
	// desired, i.e. the
	// contrast ratio of the input color and the returned luminance may not reach
	// the contrast ratio
	// asked for.
	//
	// When the desired contrast ratio and the result contrast ratio differ by more
	// than this amount,
	// an error value should be returned, or the method should be documented as
	// 'unsafe', meaning,
	// it will return a valid luminance but that luminance may not meet the
	// requested contrast ratio.
	//
	...
	// Color spaces that measure luminance, such as Y in XYZ, L* in L*a*b*, or T in
	// HCT, are known as
	// perceptually accurate color spaces.
	//
	// To be displayed, they must gamut map to a "display space", one that has a
	// defined limit on the
	// number of colors. Display spaces include sRGB, more commonly understood as
	// RGB/HSL/HSV/HSB.
	// Gamut mapping is undefined and not defined by the color space. Any gamut
	// mapping algorithm must
	...
	// contrast/a11y, maintain hue,
	// thus maintaining aesthetic intent, and reduce chroma until the color is in
	// gamut.
	//
	// HCT chooses this solution, but, that doesn't mean it will _exactly_ matched
	// desired lightness,
	// if only because RGB is quantized: RGB is expressed as a set of integers:
	// there may be an RGB
	...
	// color spaces and
	// display color spaces, methods that take a contrast ratio and luminance, and
	// return a luminance
	// that reaches that contrast ratio for the input luminance, purposefully
	// darken/lighten their
	// result such that the desired contrast ratio will be reached even if
	// inaccuracy is introduced.
	//
	// 0.4 is generous, ex. HCT requires much less delta. It was chosen because it
	// provides a rough
	// guarantee that as long as a perceptual color space gamut maps lightness such
	// that the resulting
	// lightness rounds to the same as the requested, the desired contrast ratio
	// will be reached.
	...
	/**
	 * Contrast ratio is a measure of legibility, its used to compare the lightness
	 * of two colors. This method is used commonly in industry due to its use by
	 * WCAG.
	 *
	 * <p>
	 * To compare lightness, the colors are expressed in the XYZ color space, where
	 * Y is lightness, also known as relative luminance.
	 *
	 * <p>
	...
	/**
	 * Contrast ratio of two tones. T in HCT, L* in L*a*b*. Also known as luminance
	 * or perpectual luminance.
	 *
	 * <p>
	 ...
	 * lightness. Perceptual luminance, L* in L*a*b*, T in HCT, is. Designers prefer
	 * color spaces with perceptual luminance since they're accurate to the eye.
	 *
	 * <p>
	 * Y and L* are pure functions of each other, so it possible to use perceptually
	 * accurate color spaces, and measure contrast, and measure contrast in a much
	 * more understandable way: instead of a ratio, a linear difference. This allows
	 * a designer to determine what they need to adjust a color's lightness to in
	 * order to reach their desired contrast, instead of guessing & checking with
	 * hex codes.
	 */
	 ...
	 * input T/L*. Returns -1 if ratio cannot be achieved.
	 *
	...
	public static double lighter(double tone, double ratio) {
		...
		// Invert the contrast ratio equation to determine lighter Y given a ratio and
		// darker Y.
		...
		// NOMUTANTS--important validation step; functions it is calling may change
		// implementation.
	...
	public static double darker(double tone, double ratio) {
		...
		// Invert the contrast ratio equation to determine darker Y given a ratio and
		// lighter Y.
