...
package cn.pupperclient.libraries.material3.hct;
...
public final class Cam16 {
	...
	static final double[][] XYZ_TO_CAM16RGB = { { 0.401288, 0.650173, -0.051461 }, { -0.250268, 1.204414, 0.045854 },
			{ -0.002079, 0.048952, 0.953127 } };

	// Transforms 'cone'/'RGB' responses in CAM16 to XYZ color space coordinates.
	static final double[][] CAM16RGB_TO_XYZ = { { 1.8620678, -1.0112547, 0.14918678 },
			{ 0.38752654, 0.62144744, -0.00897398 }, { -0.01584150, -0.03412294, 1.0499644 } };
	...
	// Coordinates in UCS space. Used to determine color distance, like delta E
	// equations in L*a*b*.
	 ...
	 *
	 * <p>
	 * Prefer lightness, brightness is an absolute quantity. For example, a sheet of
	 * white paper is much brighter viewed in sunlight than in indoor light, but it
	 * is the lightest object under any lighting.
	 */
	public double getQ() {
		return q;
	}
	 ...
	 *
	 * <p>
	 * Prefer chroma, colorfulness is an absolute quantity. For example, a yellow
	 * toy car is much more colorful outside than inside, but it has the same chroma
	 ...
	 *
	 * <p>
	 * Colorfulness in proportion to brightness. Prefer chroma, saturation measures
	 * colorfulness relative to the color's own brightness, where chroma is
	 * colorfulness relative to white.
	 */
	public double getS() {
		return s;
	}
	 ...
	 * the following combinations: - {j or q} and {c, m, or s} and hue - jstar,
	 * astar, bstar Prefer using a static method that constructs from 3 of those
	 * dimensions. This constructor is intended for those methods to use to return
	 * all possible dimensions.
	 *
	 * @param hue    for example, red, orange, yellow, green, etc.
	 * @param chroma informally, colorfulness / color intensity. like saturation in
	 *               HSL, except perceptually accurate.
	 * @param j      lightness
	...
	/**
	 * Create a CAM16 color from a color, assuming the color was viewed in default
	 * viewing conditions.
	 *
	...
	static Cam16 fromXyzInViewingConditions(double x, double y, double z, ViewingConditions viewingConditions) {
		...
		double j = 100.0
				* Math.pow(ac / viewingConditions.getAw(), viewingConditions.getC() * viewingConditions.getZ());
		double q = 4.0 / viewingConditions.getC() * Math.sqrt(j / 100.0) * (viewingConditions.getAw() + 4.0)
	 ...
	 *
	 * @param jstar             CAM16-UCS lightness.
	 * @param astar             CAM16-UCS a dimension. Like a* in L*a*b*, it is a
	 *                          Cartesian coordinate on the Y axis.
	 * @param bstar             CAM16-UCS b dimension. Like a* in L*a*b*, it is a
	 *                          Cartesian coordinate on the X axis.
	 * @param viewingConditions Information about the environment where the color
	 *                          was observed.
	 */
	public static Cam16 fromUcsInViewingConditions(double jstar, double astar, double bstar,
	...
	/**
	 * ARGB representation of the color. Assumes the color was viewed in default
	...
	double[] xyzInViewingConditions(ViewingConditions viewingConditions, double[] returnArray) {
		...
		double ac = viewingConditions.getAw()
		...
		double rC = Math.signum(rA) * (100.0 / viewingConditions.getFl()) * Math.pow(rCBase, 1.0 / 0.42);
		double gCBase = max(0, (27.13 * Math.abs(gA)) / (400.0 - Math.abs(gA)));
		double gC = Math.signum(gA) * (100.0 / viewingConditions.getFl()) * Math.pow(gCBase, 1.0 / 0.42);
		double bCBase = max(0, (27.13 * Math.abs(bA)) / (400.0 - Math.abs(bA)));
		double bC = Math.signum(bA) * (100.0 / viewingConditions.getFl()) * Math.pow(bCBase, 1.0 / 0.42);
