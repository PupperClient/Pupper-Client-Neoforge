...
package cn.pupperclient.libraries.discordipc.entities.pipe;

import cn.pupperclient.libraries.discordipc.IPCClient;
import cn.pupperclient.libraries.discordipc.entities.Callback;
...
public class UnixPipe extends Pipe {
	...
	public Packet read() throws IOException, JsonParseException {
		...
		if (ipcClient.isDebugMode() && ipcClient.isVerboseLogging()) {
			ipcClient.getCurrentLogger(LOGGER)
					.info(String.format("[DEBUG] Read Byte Data: %s with result %s", new String(d), readResult));
		}
		...
		if (ipcClient.isDebugMode() && ipcClient.isVerboseLogging()) {
			ipcClient.getCurrentLogger(LOGGER).info(
					String.format("[DEBUG] Read Reversed Byte Data: %s with result %s", new String(d), reversedResult));
		}
	...
	public void registerApp(String applicationId, String command) {
		...
		if (this.mkdir(desktopFilePath))
			ipcClient.getCurrentLogger(LOGGER)
					.warn("[DEBUG] Failed to create directory '" + desktopFilePath + "', may already exist");
		...
		String xdgMimeCommand = "xdg-mime default discord-" + applicationId + ".desktop x-scheme-handler/discord-"
				+ applicationId;
