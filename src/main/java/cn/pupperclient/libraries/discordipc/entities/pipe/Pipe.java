...
public abstract class Pipe {
    ...
    public static Pipe openPipe(IPCClient ipcClient, long clientId, HashMap<String, Callback> callbacks,
        ...
        // store some file so we can get the preferred client
        ...
        // close unused file, except skip 'any' because it's always a duplicate
    ...
    public void send(Packet.OpCode op, JsonObject data, Callback callback) {
        try {
            ...
            write(p.toBytes());
            if (ipcClient.isDebugMode()) {
                ipcClient.getCurrentLogger(LOGGER).info(String.format("[DEBUG] Sent packet: %s", p.toDecodedString()));
            }

            if (listener != null)
                listener.onPacketSent(ipcClient, p);
        } catch (IOException ex) {
            ipcClient.getCurrentLogger(LOGGER).error("Encountered an IOException while sending a packet and disconnected!");
    ...
    public Packet receive(Packet.OpCode op, byte[] data) {
        ...
        if (ipcClient.isDebugMode()) {
            ipcClient.getCurrentLogger(LOGGER).info(String.format("[DEBUG] Received packet: %s", p));
        }
