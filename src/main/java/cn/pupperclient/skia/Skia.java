...
public class Skia {
	...
	public static void drawBlur(float x, float y, float width, float height) {
		...
		if (HUDModSettings.getInstance().getBlurSetting().isEnabled()) {
			...
			drawImage(Kawaseblur.INGAME_BLUR.getTexture(), 0, 0, window.getScreenWidth(), window.getScreenHeight(), 1F,
    ...
    public static void drawIcon(String icon, float x, float y, float size, Color color) {
        Font iconFont = FontManager.getIconFont(size);
        Rect bounds = iconFont.measureText(icon);

        float iconX = x + (size - bounds.getWidth()) / 2 - bounds.getLeft();
        float iconY = y + (size - bounds.getHeight()) / 2 - bounds.getTop() + bounds.getHeight();

        drawText(icon, iconX, iconY, color, iconFont);
    }

    public static float getTextWidth(String status, Font regular) {
        return regular.measureText(status).getWidth();
    }
}
